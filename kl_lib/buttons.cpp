...
static uint32_t LocalTimer = 0; // Required to overcome 16bit timer of L151

// ========================= Postprocessor for PinSns ==========================
void ProcessButtons(PinSnsState_t *BtnState, uint32_t Len) {
    LocalTimer += SNS_POLL_PERIOD_MS;
...
#endif
#if BTN_REPEAT
            RepeatTimer = LocalTimer;   // Save current time
#endif
        } // if press
#endif
...
//                Uart.Printf("Elapsed %u\r", chVTTimeElapsedSinceX(LongPressTimer));
            ...
            if(!IsRepeating[i]) {
                if((LocalTimer - RepeatTimer) >= BTN_DELAY_BEFORE_REPEAT_MS) {
                    RepeatTimer = LocalTimer;
                    IsRepeating[i] = true;
                    AddEvtToQueue(beRepeat, i);
                }
            }
            else {
                if((LocalTimer - RepeatTimer) >= BTN_REPEAT_PERIOD_MS) {
                    RepeatTimer = LocalTimer;
                    AddEvtToQueue(beRepeat, i);
                }
            }
