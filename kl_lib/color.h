...
// If Color.Brt == RANDOM_CLR_BRT then color should be random.
...
class Color_t {
private:
    __attribute__((__always_inline__)) uint8_t  SetSingleBrt(int32_t v, const int32_t abrt, const int32_t abrt_max) const { return (v * abrt) / abrt_max; }
    __attribute__((__always_inline__)) uint32_t MixSingleAvg(uint32_t fore, uint32_t back) const { return (fore + back) / 2; }
    __attribute__((__always_inline__)) uint32_t MixSingleAdd(uint32_t fore, uint32_t back) const {
...
public:
    union {
        ...
        struct {
            uint8_t R, G, B;
            union { uint8_t Brt, W; }; // This class supports either RGB+Brightness or RGBW, but not both simultaneously
        };
    };
    ...
    bool IsEqualRGB(uint8_t red, uint8_t green, uint8_t blue) const { return (R == red and G == green and B == blue); }
...
#pragma region // ==== HSV ====
    ...
    static constexpr Color_t FromHSV_constexpr(uint16_t H, uint8_t S, uint8_t V) {
        // Replicate FromHSV logic, but return a Color_t
        int32_t C = (static_cast<int32_t>(V) * static_cast<int32_t>(S) * 255) / 10000;
        int32_t X = 60 - Abs32((H % 120) - 60);
        X = (C * X) / 60;
        int32_t m = ((static_cast<int32_t>(V) * 255) / 100) - C;
        int32_t R_val, G_val, B_val;
        if      (H <  60) { R_val = C + m; G_val = X + m; B_val = m; }
        else if (H < 120) { R_val = X + m; G_val = C + m; B_val = m; }
        else if (H < 180) { R_val = m;     G_val = C + m; B_val = X + m; }
        else if (H < 240) { R_val = m;     G_val = X + m; B_val = C + m; }
        else if (H < 300) { R_val = X + m; G_val = m;     B_val = C + m; }
        else              { R_val = C + m; G_val = m;     B_val = X + m; }
...
#pragma region // ==== 16-bit 565 ==== RRRRR GGGGGG BBBBB
    ...
    constexpr uint16_t To565() const { return ((R & 0xF8) << 8) | ((G & 0xFC) << 3) | (B >> 3); }
    // Byte-swapped 565 value, for displays/controllers that expect the 565
    // word transmitted low-byte-first (e.g. ST7789 over SPI in 16-bit mode).
    // GCC folds this shift/or idiom into a single ARM REV16 instruction at -Os.
    __attribute__((__always_inline__)) inline uint16_t To565Swapped() const {
        uint16_t c = To565();
        return (uint16_t)((c >> 8) | (c << 8));
    }
#pragma endregion

#pragma region // ==== Mixage ====
...
#pragma region // ==== Set RGB(WBrightness ====
...
#pragma endregion
    ...
    // Create a gradient of N colors from hue H0 to H1 (S and V fixed at 100%)
    template<int32_t N>
    static constexpr std::array<Color_t, N> MakeGradientHSV(int32_t H0, int32_t H1) {
        std::array<Color_t, N> table{};
        if constexpr (N == 0) return table;
        for (int32_t i = 0; i < N; ++i) {
            int32_t h = Proportion<int32_t>(0, (N - 1), H0, H1, i);
            h = (h % 360 + 360) % 360;
            table[i] = FromHSV_constexpr(static_cast<uint16_t>(h), 100, 100);
        }
        return table;
    }

#pragma region // ==== Constructors and printing ====
    void PrintRGBW() {
        if(IsRandom()) Printf("{random}");
        else Printf("{%u, %u, %u; %u}", R, G, B, Brt);
    }
    void PrintRGB(const char* end = "") { Printf("{%u, %u, %u}%S", R, G, B, end); }

    constexpr Color_t() : dword32(0) {}
    constexpr Color_t(uint8_t ar, uint8_t ag, uint8_t ab) : Color_t(ar, ag, ab, 0) {}  // delegate to 4-arg
    constexpr Color_t(uint8_t ar, uint8_t ag, uint8_t ab, uint8_t abrt) :
        dword32((static_cast<uint32_t>(abrt) << 24) |
                (static_cast<uint32_t>(ab)   << 16) |
                (static_cast<uint32_t>(ag)   << 8)  |
                (static_cast<uint32_t>(ar))) {}
#pragma endregion
};

static_assert(sizeof(Color_t) == 4, "Color_t must be 4 bytes");
static_assert(alignof(Color_t) == 4, "Color_t must be 4-byte aligned");
static_assert(std::is_trivially_copyable_v<Color_t>, "Color_t must be trivially copyable");
#pragma endregion
...
//        Printf(" Adjust dH %i (%u %u)\t sH %u\t sS %i (%u %u)\t sV %i (%u %u)\r", dH, H, target.H, sH, target.S-S, S, target.S, target.V-V, V, target.V);
    ...
    void AsjustHS(const ColorHSV_t &target) {
        int16_t dH = target.H - H;
        uint16_t sH = Abs32(dH);
        if (sH > 180) sH = 360 - sH;
        // Change H
...
#pragma region // ============================== HSV Adjuster ===========================
class HSVAdjuster {
private:
    int32_t startH, startS, startV;
    int32_t endH, endS, endV;
    int32_t step = 0, step_cnt = 0;
    bool hue_wrap = false;
    int32_t hue_wrap_offset = 0;
    int32_t smooth = 0;

    int32_t IProcessHAndGetDist() {
        int32_t dH = endH - startH;
        int32_t abs_dH = Abs32(dH);
        hue_wrap = (abs_dH > 180);
        if(hue_wrap) { // Determine wrap direction
            if(dH > 0) hue_wrap_offset = -360; // Go backwards through wrap
            else       hue_wrap_offset =  360; // Go forwards through wrap
            abs_dH = 360 - abs_dH;
        }
        else hue_wrap_offset = 0;
        return abs_dH;
    }
    int32_t IProcessSAndGetDist() { return Abs32(endS - startS); }
    int32_t IProcessVAndGetDist() { return Abs32(endV - startV); }

    void IStartHS() {
        step = 0;
        int32_t abs_dH = IProcessHAndGetDist();
        int32_t abs_dS = IProcessSAndGetDist();
        // Use the maximum difference for step count
        step_cnt = (abs_dH > abs_dS) ? abs_dH : abs_dS;
    }
    ...
    void IStartHSV() {
        ...
        int32_t abs_dH = IProcessHAndGetDist();
        int32_t abs_dS = IProcessSAndGetDist();
        int32_t abs_dV = IProcessVAndGetDist();
        step_cnt = (abs_dH > abs_dS) ? abs_dH : abs_dS;
        if(step_cnt < abs_dV) step_cnt = abs_dV;
    }

    int32_t IAdjustH() { // Interpolate hue with wrap handling
        if(hue_wrap) {
            int32_t wrappedH = startH + hue_wrap_offset;
            int32_t tempH = wrappedH + ((endH - wrappedH) * step) / step_cnt;
            return (tempH + 360) % 360; // Normalize to 0-359
        }
        else return startH + ((endH - startH) * step) / step_cnt;
    }
    int32_t IAdjustS() { return startS + ((endS - startS) * step) / step_cnt; } // Interpolate saturation (linear)
    int32_t IAdjustV() { return startV + ((endV - startV) * step) / step_cnt; } // Interpolate value (linear)

public:
    // For H & S adjustment only
    void StartHS(int32_t startH, int32_t startS, int32_t endH, int32_t endS) {
        this->startH = startH; this->startS = startS;
        this->endH = endH; this->endS = endS;
        IStartHS();
    }

    // For V adjustment only
    void StartVSmooth(int32_t endV, int32_t smooth) {
        this->endV = endV;
        this->smooth = smooth;
    }

    void StartHSV(int32_t startH, int32_t startS, int32_t startV, int32_t endH, int32_t endS, int32_t endV) {
        this->startH = startH; this->startS = startS; this->startV = startV;
        this->endH = endH; this->endS = endS; this->endV = endV;
        IStartHSV();
    }

    void StartHSV(ColorHSV_t start, ColorHSV_t end) {
        this->startH = start.H; this->startS = start.S; this->startV = start.V;
        this->endH = end.H; this->endS = end.S; this->endV = end.V;
        IStartHSV();
    }

    // Make next step and return true if end reached
    retv AdjustHS(ColorHSV_t &clr) {
        if(step >= step_cnt) {
            clr.H = endH;
            clr.S = endS;
            return retv::Same;
        ...
        clr.H = IAdjustH();
        step++;
        return retv::InProgress;
    }

    // Make next step and return true if end reached
    retv AdjustV(ColorHSV_t &clr) {
        if(clr.V == endV) return retv::Same;
        if(clr.V < endV) clr.V++;
        else clr.V--;
        return retv::InProgress;
    }

    retv AdjustHSV(ColorHSV_t &clr) {
        if(step >= step_cnt) {
            clr.H = endH;
            clr.S = endS;
            clr.V = endV;
    ...
    uint32_t CalcDelayByV(ColorHSV_t &clr) { return ClrCalcDelay(clr.V, smooth); }
};
#pragma endregion
