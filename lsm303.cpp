...
bool LSM303::init(deviceType device, sa0State sa0) {
    ...
    if (device == device_auto) {
        ...
        else if (testReg(NON_D_ACC_SA0_HIGH_ADDRESS,
                CTRL_REG1_A) != TEST_REG_NACK) {
        ...
        else if (testReg(NON_D_ACC_SA0_LOW_ADDRESS,
                CTRL_REG1_A) != TEST_REG_NACK) {
    ...
    if (sa0 == sa0_auto) {
        if (device == device_D) {
            ...
            if (testReg(NON_D_ACC_SA0_HIGH_ADDRESS,
                    CTRL_REG1_A) != TEST_REG_NACK) {
                sa0 = sa0_high;
            } else if (testReg(NON_D_ACC_SA0_LOW_ADDRESS,
                    CTRL_REG1_A) != TEST_REG_NACK) {
    ...
    switch (device) {
        ...
        case device_DLM:
            acc_address =
                    (sa0 == sa0_high) ? NON_D_ACC_SA0_HIGH_ADDRESS :
                            NON_D_ACC_SA0_LOW_ADDRESS;
            mag_address = NON_D_MAG_ADDRESS;
...
void LSM303::writeAccReg(regAddr reg, uint8_t value) {
    uint8_t Buf[2];
    Buf[0] = (uint8_t)reg;
    Buf[1] = value;
    last_status = i2cimu.WriteWrite(acc_address, Buf, 2, nullptr, 0);
    Uart.Printf("WAReg=%X; %X; r=%u\r\n", reg, value, last_status);
...
uint8_t LSM303::readAccReg(regAddr reg) {
    uint8_t value=0;
    uint8_t r = (uint8_t)reg;
    last_status = i2cimu.WriteRead(acc_address, &r, 1, &value, 1);
    Uart.Printf("RAReg=%X; %X; r=%u\r\n", reg, value, last_status);
//    Wire.beginTransmission(acc_address);
//    Wire.write((uint8_t) reg);
//    last_status = Wire.endTransmission();
//    Wire.requestFrom(acc_address, (uint8_t) 1);
//    value = Wire.read();
...
void LSM303::writeMagReg(regAddr reg, uint8_t value) {
    uint8_t Buf[2];
    Buf[0] = (uint8_t)reg;
    Buf[1] = value;
    last_status = i2cimu.WriteWrite(mag_address, Buf, 2, nullptr, 0);
    Uart.Printf("WMReg=%X; %X; r=%u\r\n", reg, value, last_status);
...
uint8_t LSM303::readMagReg(regAddr reg) {
    // if dummy register address (magnetometer Y/Z), look up actual translated address (based on device type)
    if (reg < 0) reg = translated_regs[-reg];
    uint8_t value=0;
    uint8_t r = (uint8_t)reg;
    last_status = i2cimu.WriteRead(mag_address, &r, 1, &value, 1);
    Uart.Printf("RMReg=%X; %X; r=%u\r\n", reg, value, last_status);
//    Wire.beginTransmission(mag_address);
//    Wire.write((uint8_t) reg);
//    last_status = Wire.endTransmission();
//    Wire.requestFrom(mag_address, (uint8_t) 1);
//    value = Wire.read();
//    Wire.endTransmission();
...
void LSM303::writeReg(regAddr reg, uint8_t value) {
...
uint8_t LSM303::readReg(regAddr reg) {
    // mag address == acc_address for LSM303D, so it doesn't really matter which one we use.
    // Use writeMagReg so it can translate OUT_[XYZ]_[HL]_M
    if (_device == device_D || reg < CTRL_REG1_A) {
...
void LSM303::readAcc(void) {
    ...
    last_status = i2cimu.WriteRead(acc_address, &reg, 1, (uint8_t*)&a, 6);
//
//    Wire.beginTransmission(acc_address);
...
void LSM303::readMag(void) {
    ...
    uint8_t reg = (_device == device_D) ? translated_regs[-OUT_X_L_M] | (1 << 7) : translated_regs[-OUT_X_H_M];
    last_status = i2cimu.WriteRead(mag_address, &reg, 1, (uint8_t*)&m, 6);
    if(_device != device_D) {
        m.x = (int16_t)__REV16((uint16_t)m.x);
        m.y = (int16_t)__REV16((uint16_t)m.y);
        m.z = (int16_t)__REV16((uint16_t)m.z);
...
//
//    // compute heading
//    float heading = atan2(vector_dot(&E, &from), vector_dot(&N, &from)) * 180
//            / M_PI;
//    if (heading < 0)
//        heading += 360;
//    return heading;
//}
//
//template<typename Ta, typename Tb, typename To> void LSM303::vector_cross(
...
//}
//
//template<typename Ta, typename Tb> float LSM303::vector_dot(const vector<Ta> *a,
//        const vector<Tb> *b) {
...
int LSM303::testReg(uint8_t address, regAddr reg) {
    uint8_t value=0;
    uint8_t r = (uint8_t)reg;
    last_status = i2cimu.WriteRead(address, &r, 1, &value, 1);
    Uart.Printf("tst addr=%X; Reg=%X; %X; r=%u\r\n", address, reg, value, last_status);
    if(last_status == OK) return value;
    else return TEST_REG_NACK;
